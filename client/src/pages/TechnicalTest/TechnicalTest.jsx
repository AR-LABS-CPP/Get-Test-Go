...
//         // check if the time is over and if so
...
const TechnicalTest = (props) => {
    ...
    const [buttonText, setButtonText] = useState("Next");
    const [errorText, setErrorText] = useState("");
    const [assessmentName, setAssessmentName] = useState(props.assessmentName)
    const [secondsLeft, setSecondsLeft] = useState(10);
    const candidateAnswersRef = useRef([]);
    const assessmentInfo = {
        sectionName: `TECHNICAL / ${props.assessmentName}`,
        totalQuestions: props.totalQuestions,
    };
    ...
    const handleNextQuestion = () => {
        if (buttonText === "Finish") {
            ...
            if (
                questions[activeQuestion].question_type_name === "MCQ" &&
                mcqselectedOption === ""
            ) {
                setErrorText("Please select an option");
            } else {
                candidateAnswersRef.current.push(getSelectedOption());
                setActiveQuestion((prevValue) => prevValue + 1);
            }
        }
    };
    ...
    const handleFinish = () => {
        candidateAnswersRef.current.push(getSelectedOption());
        props.storeAnswersCB(assessmentName, candidateAnswersRef.current);
        candidateAnswersRef.current = [];
        props.setActiveAssessmentCB();
    };
        ...
        if (secondsLeft === 0) {
            if(buttonText === "Finish") {
                ...
                if (
                    ...
                    questions[activeQuestion].question_type_name === "TrueFalse"
                ) {
                    candidateAnswersRef.current.push(getSelectedOption());
                } else {
                    candidateAnswersRef.current.push(null);
                }
    
                console.log(candidateAnswersRef.current)
