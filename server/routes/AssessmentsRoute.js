...
assessmentsRouter.post("/new", (req, res) => {
    assessmentModel.recruiterAssessmentExists(req.body.recruiterEmail, req.body.assessmentName).then(response => {
        if(response === true) {
            res.status(409).send("Assessment already exists")
        }
        else {
            let assessmentType = req.body.assessmentType === "GENERAL" ? 1 : 2

            assessmentModel.addNewAssessment(req.body.assessmentName, req.body.assessmentDetails, assessmentType).then(_ => {
...
assessmentsRouter.post("/question/add/mcq", (req, res) => {
    assessmentModel.assessmentQuestionExists(req.body.assessmentName, req.body.assessmentQuestion).then(response => {
        if(response === false) {
            assessmentModel.addMCQ(req.body.assessmentName, req.body.assessmentQuestion, req.body.optionOne, req.body.optionTwo, req.body.optionThree, req.body.optionFour, req.body.correctOption).then(response => {
                res.status(200).send(response)
            }).catch(error => {
                res.status(500).send(error)
            })
